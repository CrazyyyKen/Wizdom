  ...
  if (userBagJSON) {
    ...
    userBag.forEach((book, index) => {
      ...
      row.innerHTML = `
                ...
                <td><img src="${book.coverSrc}" alt="${
        book.name
      } Cover" style="height: 100px; width: auto;"></td>
                <td>${book.name}</td>
                <td>RM10</td>
                <td>
...
document
  .querySelector(".table #bagTableBody")
...
checkoutButton.addEventListener("click", () => {
  ...
  if (!userBagJSON || JSON.parse(userBagJSON).length === 0) {
    ...
    bookNamesElement.innerHTML = userBag
      .map((book) => book.name + "<br>")
      .join("");
    ...
    const payAndRentButton = document.querySelector(
